...
const generateAIStudyPlan = async (req, res) => {
  try {
    ...
    // 2. Prepare context for AI with IDs
    const subjectsDetails = subjects
      .map((s) => {
        const topicsList = s.topics.map((t) => `${t.name} (Difficulty: ${t.difficulty}/5)`).join(", ");
        return `- ID: ${s.id}, Name: ${s.name}: ${topicsList} (Overall Subject Difficulty: ${s.difficulty}/5, Priority: ${s.priority}/5)`;
      })
      .join("\n");
...
Subjects & Topics (Use the provided IDs):
${subjectsDetails}
